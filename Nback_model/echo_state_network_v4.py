...
class SpikingEchoStateNetwork:
    ...
    def __init__(self, n_inputs, n_reservoir, n_outputs, p_rewire,
                 ...
                 A_minus=0.5, tau_plus=50.0, tau_minus=50.0, 
                 decay_factor=0.9): # 最適化 #tau大きいほど減衰遅くなる
        ...
        self.input_region_size = 100
        self.output_region_size = 100
        ...
        self.W_feedback = np.random.rand(self.hidden_region_size, n_outputs) - 0.5  # フィードバック重み行列
    ...
    def update(self, input_data, current_time, dt=1):
        ...
        if len(self.previous_inputs) >= 2:
            past_input = self.previous_inputs[-2] * self.decay_factor
            input_data = input_data + past_input
        ...
        if len(self.previous_inputs) > 0:
            last_output = np.dot(self.W_output, self.spikes[-self.output_region_size:])
            feedback = np.dot(self.W_feedback, last_output)
            self.membrane_potential[self.input_region_size:self.input_region_size + self.hidden_region_size] += feedback
        ...
        # 現在の入力を保持し、過去の入力を減衰させる
        if len(self.previous_inputs) >= 2:
            self.previous_inputs[-2] *= self.decay_factor
        self.previous_inputs.append(input_data)
    ...
    def predict(self, input_data):
        spikes = self.update(input_data, 0)
        output_spikes = spikes[-self.output_region_size:]
        return np.dot(self.W_output, output_spikes)
